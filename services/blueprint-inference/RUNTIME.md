...
- `BOS_RASTER2SEQ_CHECKPOINT` must identify the exact commercially approved B.O.S. checkpoint. There is no approved third-party default.
- `BOS_RASTER2SEQ_CHECKPOINT_MANIFEST` must point to its governance manifest. The API adapter remains unavailable when the manifest is missing, incomplete, or unapproved.
...
CAGE remains blocked from commercial B.O.S. inference under its published license. MitUNet-style research remains disabled until a concrete implementation/checkpoint license is verified. Raster2Seq, SAM 2.1, and RoomFormer architectures are governed independently by `model_governance.py`; no architecture license by itself authorizes pretrained weights.
