...
export function FieldPhotoCapture({ projectId, projectName, onUploaded }: { projectId: string; projectName: string; onUploaded: (photo: FieldPhotoUpload) => Promise<void> }) {
  ...
  const upload = async () => {
    ...
    if (!workspace.context) { setMessage(workspace.errorMessage); setBusy(false); uploadInFlightRef.current = false; return; }
    ...
    if (stored.error) { setMessage(stored.error.message); setBusy(false); uploadInFlightRef.current = false; return; }
    ...
    if (saved.error) { await supabase.storage.from(BUCKET).remove([path]); setMessage(saved.error.message); setBusy(false); uploadInFlightRef.current = false; return; }
    setFile(null); setNote(""); setMessage("Field photo uploaded and attached to the daily report."); setBusy(false); uploadInFlightRef.current = false; await onUploaded({ id: photoId, fileName: file.name, category });
  };
