  ...
  return (
    <div className="space-y-5">
      ...
      <div className="grid gap-5 xl:grid-cols-[1.3fr_0.7fr]">
        <Card as="section" variant="elevated" className="rounded-[16px] border border-[var(--bos-border-light)]">
          <CardHeader className="bg-[var(--color-surface-subtle)]/55"><CardTitle className="text-section-title">Cost Code Variance</CardTitle></CardHeader>
          <CardContent className="overflow-x-auto p-0">
            <table className="min-w-full border-collapse text-sm">
              <thead><tr className="border-b border-[var(--bos-border-light)] bg-[var(--color-neutral-50)] text-left text-xs uppercase tracking-[0.08em] text-[var(--bos-text-medium-on-light)]"><th className="px-3 py-2.5">Cost Code</th><th className="px-3 py-2.5">Budget</th><th className="px-3 py-2.5">Committed</th><th className="px-3 py-2.5">Actual</th><th className="px-3 py-2.5">Forecast</th><th className="px-3 py-2.5">Variance $</th><th className="px-3 py-2.5">Variance %</th></tr></thead>
              <tbody>
                {report.costCodeVariance.length === 0 ? <tr><td colSpan={7} className="px-3 py-3 text-sm text-[var(--bos-text-medium-on-light)]">No project-scoped cost code procurement activity is available yet.</td></tr> : report.costCodeVariance.slice(0, 20).map((row) => (
                  <tr key={row.costCodeId} className="border-b border-[var(--bos-border-light)] text-[var(--bos-text-strong-on-light)]"><td className="px-3 py-2.5 font-semibold">{row.code} {row.name}</td><td className="px-3 py-2.5">{formatMoney(row.budget)}</td><td className="px-3 py-2.5">{formatMoney(row.committed)}</td><td className="px-3 py-2.5">{formatMoney(row.actual)}</td><td className="px-3 py-2.5">{formatMoney(row.forecast)}</td><td className="px-3 py-2.5">{formatMoney(row.varianceAmount)}</td><td className="px-3 py-2.5">{formatPercent(row.variancePercent)}</td></tr>
                ))}
              </tbody>
            </table>
          </CardContent>
        ...
        <Card as="section" variant="elevated" className="rounded-[16px] border border-[var(--bos-border-light)]">
          <CardHeader className="bg-[var(--color-surface-subtle)]/55"><CardTitle className="text-section-title">Billing & Receivables</CardTitle></CardHeader>
      ...
      <Card as="section" variant="elevated" className="rounded-[16px] border border-[var(--bos-border-light)]">
        <CardHeader className="bg-[var(--color-surface-subtle)]/55"><CardTitle className="text-section-title">Data Availability</CardTitle></CardHeader>
        <CardContent className="space-y-2.5 p-4">
          {report.availability.map((item) => (
            <div key={item.key} className="rounded-[10px] border border-[var(--bos-border-light)] bg-[var(--color-neutral-50)] px-3 py-2.5">
              <div className="flex flex-wrap items-center justify-between gap-2"><p className="text-sm font-semibold text-[var(--bos-text-strong-on-light)]">{item.label}</p><span className={`inline-flex rounded-full px-2 py-1 text-xs font-semibold ${availabilityClass(item.status)}`}>{toTitleCase(item.status)}</span></div>
...
function MetricCard({ label, value, context }: { label: string; value: string; context: string }) {
  return <article className="rounded-[14px] border border-[var(--bos-border-light)] bg-[linear-gradient(180deg,var(--bos-bg-workspace-card),var(--color-neutral-50))] p-4 shadow-[var(--bos-shadow-workspace-card)]"><p className="text-xs font-bold uppercase tracking-[0.08em] text-[var(--bos-text-medium-on-light)]">{label}</p><p className="mt-2 text-2xl font-bold text-[var(--bos-text-strong-on-light)]">{value}</p><p className="mt-1 text-xs text-[var(--bos-text-medium-on-light)]">{context}</p></article>;
}
...
function toStatusLabel(status: "on_track" | "at_risk" | "over_budget" | "unavailable") { if (status === "on_track") return "On Track"; if (status === "at_risk") return "At Risk"; if (status === "over_budget") return "Over Budget"; return "Unavailable"; }
function statusClass(status: "on_track" | "at_risk" | "over_budget" | "unavailable") { if (status === "on_track") return "bg-[var(--color-success-100)] text-[var(--color-success-700)]"; if (status === "at_risk") return "bg-[var(--color-warning-100)] text-[var(--color-warning-700)]"; if (status === "over_budget") return "bg-[var(--color-danger-100)] text-[var(--color-danger-700)]"; return "bg-[var(--color-neutral-100)] text-[var(--color-text-secondary)]"; }
function availabilityClass(status: "available" | "partial" | "unavailable") { if (status === "available") return "bg-[var(--color-success-100)] text-[var(--color-success-700)]"; if (status === "partial") return "bg-[var(--color-warning-100)] text-[var(--color-warning-700)]"; return "bg-[var(--color-danger-100)] text-[var(--color-danger-700)]"; }
