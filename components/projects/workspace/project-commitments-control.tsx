...
export function ProjectCommitmentsControl({ projectId, companyId, budget }: { projectId: string; companyId: string; budget: number | null }) {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      const [laborResult, subcontractResult, timeResult] = await Promise.all([laborQuery, subcontractQuery, timeQuery]);
      if (!active) return;
      ...
      else if (subcontractResult.error || timeResult.error) setNotice("Some live job-cost details could not be loaded.");
      setLoading(false);
    }
    void load();
    return () => { active = false; };
  ...
  return <section id="project-commitments" className="rounded-[18px] border border-[var(--bos-border-light)] bg-[var(--bos-bg-workspace-surface)] p-4 shadow-[var(--shadow-small)] sm:p-5" aria-labelledby="project-commitments-title">
    <div className="flex flex-wrap items-start justify-between gap-3"><div><h2 id="project-commitments-title" className="text-lg font-extrabold text-[var(--bos-text-strong-on-light)]">Budget & Commitments</h2><p className="mt-1 text-xs font-medium text-[var(--bos-text-medium-on-light)]">Approved-time labor and signed subcontract obligations are tied back to this project budget.</p></div><Badge tone={summary.budgetRemainingAfterCommitments !== null && summary.budgetRemainingAfterCommitments < 0 ? "danger" : "info"}>{loading ? "Loading controls…" : `${money(summary.totalCommitted)} committed`}</Badge></div>
