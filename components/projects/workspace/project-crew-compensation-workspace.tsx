...
export function ProjectCrewCompensationWorkspace({ projectId, localeTag }: { projectId:string; localeTag:string }) {
  ...
  return <div className="space-y-4">
    <section className="space-y-3">
      <div className="flex flex-wrap items-center justify-between gap-3"><div><h2 className="text-xl font-extrabold text-[var(--bos-text-strong-on-light)]">Employees & Crews</h2><p className="mt-1 text-sm text-[var(--bos-text-medium-on-light)]">Project assignments, schedules, and employee compensation details.</p></div><div className="flex items-center gap-2"><span className="text-xs font-bold text-[var(--bos-text-medium-on-light)]">{assignments.length} assignment{assignments.length===1?"":"s"}</span><Link href="/crews"><Button variant="outline" size="sm">Open CrewOS</Button></Link></div></div>
    ...
    <section className="rounded-[18px] border border-[var(--bos-border-light)] bg-[var(--bos-bg-workspace-surface)] p-5 shadow-[var(--shadow-small)]">
      <div className="flex flex-wrap items-start justify-between gap-3"><div><h2 className="text-xl font-extrabold text-[var(--bos-text-strong-on-light)]">Labor & Subcontractor Agreements</h2><p className="mt-1 text-sm text-[var(--bos-text-medium-on-light)]">Employee labor totals and each subcontractor&apos;s compensation agreement, commitment, and authorization status.</p></div><div className="flex flex-wrap gap-2"><Link href="/invoices/payroll"><Button variant="outline" size="sm">Employee Pay Rates</Button></Link><Link href={`/projects/${projectId}?tab=subcontractors`}><Button size="sm">Manage Subcontractors</Button></Link></div></div>
