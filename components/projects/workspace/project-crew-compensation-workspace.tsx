...
export function ProjectCrewCompensationWorkspace({ projectId, localeTag }: { projectId:string; localeTag:string }) {
  ...
  return <div className="space-y-4">
    <section className="rounded-[18px] border border-[var(--bos-border-light)] bg-[var(--bos-bg-workspace-surface)] p-5 shadow-[var(--shadow-small)]">
      <div className="flex flex-wrap items-start justify-between gap-3"><div><h2 className="text-xl font-extrabold text-[var(--bos-text-strong-on-light)]">Crew & Trade Partner Costs</h2><p className="mt-1 text-sm text-[var(--bos-text-medium-on-light)]">Who is assigned, how they are paid, projected labor, approved-time actual cost, and subcontract commitments.</p></div><div className="flex gap-2"><Link href="/crews"><Button variant="outline" size="sm">Open CrewOS</Button></Link><Link href="/invoices/payroll"><Button variant="outline" size="sm">Employee pay rates</Button></Link></div></div>
    ...
    <section className="space-y-3">
      <div className="flex items-center justify-between"><h3 className="text-lg font-extrabold text-[var(--bos-text-strong-on-light)]">Employees & Crews</h3><span className="text-xs font-bold text-[var(--bos-text-medium-on-light)]">{assignments.length} assignment{assignments.length===1?"":"s"}</span></div>
