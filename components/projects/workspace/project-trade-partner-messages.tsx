  ...
  return (
    <Card className="overflow-hidden border-[var(--bos-border-light)] bg-[linear-gradient(180deg,var(--bos-bg-workspace-card),var(--color-neutral-50))] shadow-[var(--bos-shadow-workspace-card)]" data-project-trade-partner-messages>
      ...
      <CardContent className="p-0">
        ...
        <div className="max-h-[28rem] min-h-64 space-y-3 overflow-y-auto bg-[var(--bos-bg-workspace-card)] p-4" aria-live="polite">
          {isLoading ? <SkeletonLoader className="h-36 w-full" /> : messages.length ? messages.map((message) => <article key={message.id} className={`max-w-[86%] rounded-2xl px-4 py-3 shadow-sm ${message.sender_type === "internal" ? "ml-auto bg-[var(--color-brand-700)] text-white" : "border border-[var(--bos-border-light)] bg-white text-[var(--bos-text-strong-on-light)]"}`}><p className="whitespace-pre-wrap text-sm font-medium leading-6">{message.body}</p><p className={`mt-1 text-[10px] font-semibold ${message.sender_type === "internal" ? "text-blue-100" : "text-[var(--bos-text-medium-on-light)]"}`}>{message.sender_type === "trade_partner" ? "Trade Partner" : message.is_mine ? "You" : "B.O.S. Team"} · {formatDateTime(message.created_at)}</p></article>) : <div className="flex min-h-52 items-center justify-center rounded-xl border border-dashed border-[var(--bos-border-light)] bg-white p-6 text-center text-sm font-medium text-[var(--bos-text-medium-on-light)]">No messages yet. Start the project conversation below.</div>}
        </div>
        ...
        {canManage ? <div className="border-t border-[var(--bos-border-light)] bg-white p-4"><label htmlFor="project-trade-partner-message" className="text-sm font-bold text-[var(--bos-text-strong-on-light)]">Message {selected?.trade_name}</label><textarea id="project-trade-partner-message" value={draft} onChange={(event) => setDraft(event.currentTarget.value)} onKeyDown={(event) => { if ((event.metaKey || event.ctrlKey) && event.key === "Enter") { event.preventDefault(); void sendMessage(); } }} maxLength={4000} rows={3} placeholder="Type a project message…" className="mt-2 w-full rounded-xl border border-[var(--bos-border-default)] bg-[var(--bos-bg-control)] p-3 text-sm font-medium text-[var(--bos-text-primary)] placeholder:text-[var(--bos-text-muted)]" /><div className="mt-2 flex items-center justify-between gap-3"><p className="text-xs font-medium text-[var(--bos-text-medium-on-light)]">The assigned project manager is notified when the trade partner replies.</p><Button type="button" disabled={!draft.trim() || isSending} onClick={() => void sendMessage()}>{isSending ? "Sending…" : <><Send size={15} aria-hidden="true" />Send Message</>}</Button></div></div> : <p className="border-t border-[var(--bos-border-light)] bg-white p-4 text-sm font-medium text-[var(--bos-text-medium-on-light)]">You have read-only access to this conversation.</p>}
      </CardContent>
    </Card>
  );
}
