...
export function WorkspaceHeader({
  ...
  return (
    <Card as="section" variant="elevated" className={["min-w-0 max-w-full overflow-hidden rounded-[20px] border-[var(--workspace-header-border)] bg-[var(--workspace-header-surface)] shadow-[0_28px_52px_-30px_rgba(2,6,17,0.88)]", className].filter(Boolean).join(" ")}>
...
export function WorkspaceHero({
  ...
  return (
    <Card as="section" variant="elevated" className={["group min-w-0 max-w-full overflow-hidden rounded-[20px] border-[var(--workspace-hero-border)] bg-[var(--workspace-hero-surface)] shadow-[0_26px_56px_-30px_rgba(2,6,17,0.88)]", className].filter(Boolean).join(" ")}>
      <CardContent className="grid min-w-0 gap-6 p-5 lg:grid-cols-[1.3fr_minmax(0,1fr)] lg:p-7">
        <div>{media}</div>
        <section className="flex min-w-0 h-full flex-col justify-between rounded-[18px] border border-[var(--workspace-hero-panel-border)] bg-[var(--workspace-hero-panel-surface)] p-5 text-[var(--workspace-hero-panel-text)] shadow-[inset_0_1px_0_rgba(255,255,255,0.08)] md:p-6">
          <div className="space-y-5">
            {badgeLabel ? <Badge tone={badgeTone} className="border border-[var(--workspace-hero-badge-border)] bg-[var(--workspace-hero-badge-surface)] px-3 py-1 text-xs font-bold tracking-[0.05em] text-white">{badgeLabel}</Badge> : null}
...
export function WorkspaceTabs({
  ...
  return (
    <section className="min-w-0 max-w-full overflow-hidden rounded-[20px] border border-[var(--workspace-tabs-border)] bg-[var(--workspace-tabs-surface)] p-2.5 shadow-[0_18px_32px_-22px_rgba(3,7,18,0.72)]">
          ...
          return (
            <button
              ...
              className={`group whitespace-nowrap rounded-[12px] border px-3.5 py-2.5 text-[0.82rem] font-semibold tracking-[0.01em] transition-all duration-200 ease-out focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-[var(--focus-ring-primary)] ${
                active
                  ? "border-[var(--workspace-tab-active-border)] bg-[var(--workspace-tab-active-surface)] text-white shadow-[0_8px_16px_-12px_rgba(30,120,255,0.7)]"
