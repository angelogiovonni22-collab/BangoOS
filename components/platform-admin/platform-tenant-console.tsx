  ...
  return (
    <article className="overflow-hidden rounded-[var(--radius-card)] border border-[var(--color-border-strong)] bg-[var(--color-surface-card)] shadow-[var(--shadow-small)]">
      <div className="flex flex-col gap-3 border-b border-[var(--color-border-subtle)] bg-[var(--color-surface-subtle)] px-5 py-4 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="flex flex-wrap gap-2 text-xs font-bold"><span className="rounded-full bg-blue-100 px-3 py-1 text-blue-800">{title(tenant.planKey)}</span><span className="rounded-full bg-emerald-100 px-3 py-1 text-emerald-800">{title(tenant.lifecycleStatus)}</span></div>
      </div>
      <div className="grid gap-4 p-5 lg:grid-cols-3">
        <div className="grid grid-cols-2 gap-3 rounded-[var(--radius-control)] border border-[var(--color-border-subtle)] p-4 text-sm"><Metric label="Members" value={`${tenant.memberCount}/${tenant.seatLimit}`} /><Metric label="Projects" value={String(tenant.projectCount)} /><Metric label="Text allowance" value={tenant.orionTextAllowance.toLocaleString()} /><Metric label="Voice minutes" value={tenant.orionVoiceMinutes.toLocaleString()} /></div>
...
function Metric({ label, value }: { label: string; value: string }) { return <div><p className="text-xs font-semibold uppercase tracking-[0.08em] text-[var(--color-text-secondary)]">{label}</p><p className="mt-1 text-xl font-bold text-[var(--color-text-primary)]">{value}</p></div>; }
function Field({ label, children }: { label: string; children: React.ReactNode }) { return <label className="text-xs font-bold uppercase tracking-[0.08em] text-[var(--color-text-secondary)]"><span>{label}</span><span className="mt-1 block [&_input]:h-10 [&_input]:w-full [&_input]:rounded-[var(--radius-control)] [&_input]:border [&_input]:border-[var(--color-border-strong)] [&_input]:bg-[var(--color-surface-subtle)] [&_input]:px-3 [&_input]:text-sm [&_input]:font-medium [&_input]:normal-case [&_input]:tracking-normal [&_input]:text-[var(--color-text-primary)] [&_select]:h-10 [&_select]:w-full [&_select]:rounded-[var(--radius-control)] [&_select]:border [&_select]:border-[var(--color-border-strong)] [&_select]:bg-[var(--color-surface-subtle)] [&_select]:px-3 [&_select]:text-sm [&_select]:font-medium [&_select]:normal-case [&_select]:tracking-normal [&_select]:text-[var(--color-text-primary)]">{children}</span></label>; }
