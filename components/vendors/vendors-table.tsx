...
export function VendorsTable({ items, total, page, pageSize, onPageChange }: VendorsTableProps) {
  ...
  return (
    <TableContainer
      title={`${vendorLabel} Directory`}
      description={`Manage ${vendorsLabel.toLowerCase()} records, performance, and payment preferences.`}
    >
      <EnterpriseTable ariaLabel={`${vendorLabel} directory table`}>
        <EnterpriseTableHead>
          <tr>
            <EnterpriseTableHeading>{vendorLabel}</EnterpriseTableHeading>
        ...
        <EnterpriseTableBody>
          {items.map((vendor) => (
            <EnterpriseTableRow
              ...
              role="link"
              tabIndex={0}
              aria-label={`Open ${vendorLabel} ${vendor.displayName}`}
              onClick={(event) => {
            ...
            >
              <EnterpriseTableCell>
                <div className="flex items-center gap-2">
                  ...
                  {vendor.preferredVendor ? <Star size={14} className="fill-amber-400 text-amber-500" aria-label={`Preferred ${vendorLabel}`} /> : null}
                </div>
              ...
              <EnterpriseTableCell align="right">
                <div className="inline-flex items-center gap-1">
                  <IconLink href={`/vendors/${vendor.id}`} icon={<Eye size={15} />} label={`View ${vendorLabel}`} variant="ghost" size="sm" />
                  <IconLink href={`/vendors/${vendor.id}/edit`} icon={<Pencil size={15} />} label={`Edit ${vendorLabel}`} variant="ghost" size="sm" />
                </div>
              </EnterpriseTableCell>
            </EnterpriseTableRow>
          ))}
