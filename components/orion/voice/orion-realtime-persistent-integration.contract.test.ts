...
function main() {
  ...
  assert(unified.includes("resolveKnownOrionOperatorHref(result.href)") && unified.includes("router.push(safeHref)"), "every Realtime navigation path is canonicalized at the final browser boundary");
  ...
  assert(unified.includes("client.connect({ voice: realtimeVoice, voiceStyle, isolationMode: DEFAULT_ORION_VOICE_ISOLATION_MODE })"), "selected Realtime voice, approved style, and focused isolation are used when the session connects");
