...
export function PlansTable({
  ...
  return (
    <section className="self-start overflow-hidden rounded-[var(--radius-card)] border border-[var(--color-border-subtle)] bg-[var(--color-surface-card)] shadow-[var(--shadow-small)]">
      <EnterpriseTable ariaLabel="Plans and drawings register" minWidthClassName="min-w-[1040px]">
        <EnterpriseTableHead className="text-xs uppercase tracking-[0.08em] text-[var(--color-text-muted)]">
          <tr>
            <EnterpriseTableHeading className="w-[52px] px-4">
              <input type="checkbox" checked={allSelected} onChange={(event) => onToggleSelectAll(event.target.checked)} aria-label="Select all documents" className="h-4 w-4 rounded border-[var(--color-border-strong)]" />
            </EnterpriseTableHeading>
        ...
        <EnterpriseTableBody>
          {documents.map((document) => {
            ...
            return (
              <EnterpriseTableRow key={document.id} selected={isFocused} className={`border-t border-[var(--color-border-subtle)] ${isFocused ? "shadow-[inset_3px_0_0_0_var(--color-brand-600)]" : ""}`}>
                <EnterpriseTableCell className="px-4 py-4 align-middle"><input type="checkbox" checked={isSelected} onChange={(event) => onToggleSelect(document.id, event.target.checked)} aria-label={`Select ${document.fileName}`} className="h-4 w-4 rounded border-[var(--color-border-strong)]" /></EnterpriseTableCell>
                <EnterpriseTableCell className="px-4 py-4 align-middle"><button type="button" onClick={() => onSelectDocument(document.id)} className="text-sm font-semibold text-[var(--color-text-primary)] underline-offset-4 hover:underline focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-[var(--focus-ring-primary)]">{document.fileName}</button></EnterpriseTableCell>
                ...
                <EnterpriseTableCell align="right" className="px-3 py-3 align-middle"><DocumentActions fileName={document.fileName} fileUrl={document.fileUrl} onPreview={() => onSelectDocument(document.id)} /></EnterpriseTableCell>
              </EnterpriseTableRow>
            );
          })}
        </EnterpriseTableBody>
...
function SortableHeader({ label, value, sortKey, sortDirection, onSortRequest }: { label: string; value: PlansSortKey; sortKey: PlansSortKey; sortDirection: PlansSortDirection; onSortRequest: (key: PlansSortKey) => void }) {
  const isActive = value === sortKey;
  return (
    <EnterpriseTableHeading className="px-4">
      <button type="button" onClick={() => onSortRequest(value)} className="inline-flex items-center gap-1.5 font-semibold text-[var(--color-text-muted)] hover:text-[var(--color-text-primary)] focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-[var(--focus-ring-primary)]" aria-label={`Sort by ${label}${isActive ? ` (${sortDirection})` : ""}`}>
