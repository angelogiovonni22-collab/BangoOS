...
assert(fidelityOverlay.includes("formatBlueprintFeetInches") && fidelityOverlay.includes("data-wall-dimension") && fidelityOverlay.includes("Hide feet & inches") && fidelityOverlay.includes("Show feet & inches"), "Source fidelity review must provide deterministic construction-style feet/inches wall labels with a visibility toggle");
assert(fidelityOverlay.includes("metrics.scaleConfidence >= 0.9") && fidelityOverlay.includes('graph.scale.source !== "unknown"'), "Source dimension labels must remain suppressed until scale evidence clears the verification gate");
assert(fidelityOverlay.includes("length >= 1.2192") && fidelityOverlay.includes(".slice(0, 36)"), "Source dimension labels must be bounded to meaningful wall runs to prevent unreadable overlay clutter");
assert(fidelityOverlay.includes("Wall topology still requires review before construction use"), "A needs-review graph must keep source-overlay dimensions explicitly review-gated");
