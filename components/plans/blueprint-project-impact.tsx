...
const empty: BlueprintProjectImpactSummary = { openIssues: 0, unlinkedIssues: 0, tasks: 0, punchItems: 0, changeOrders: 0, workforceAssignments: 0, estimateItems: 0 };
...
export function BlueprintProjectImpact({ companyId, projectId }: { companyId: string; projectId: string }) {
  ...
  const cards = [
    ...
    { label: "Financial impacts", value: summary.changeOrders + summary.estimateItems, icon: DollarSign, detail: `${summary.changeOrders} CO · ${summary.estimateItems} estimate` },
  ];
