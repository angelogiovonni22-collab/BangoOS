...
export function PlansPreview({ selectedDocument, projectName, onUploadRevision, companyId, projectId, userId, initialWorkspaceOpen = false, initialPage = 1, initialAnnotationId }: PlansPreviewProps) {
  ...
  if (!selectedDocument) {
    return (
      <section className="self-start rounded-[var(--radius-2xl)] border border-[var(--color-border-subtle)] bg-[var(--color-surface-card)] shadow-[var(--shadow-small)]">
        <EmptyState compact icon="P" title="Plan preview unavailable" description="Select a connected plan file to open its preview and drawing tools." />
      </section>
    );
  }
    ...
    <section className="self-start rounded-[var(--radius-2xl)] border border-[var(--color-border-subtle)] bg-[var(--color-surface-card)] shadow-[var(--shadow-small)]" aria-label="Drawing preview and metadata">
      ...
      <div className="space-y-5 p-5">
        {selectedDocument.fileUrl && (previewType === "ifc" || previewType === "gltf") ? (
          ...
          <div className="flex h-72 items-center justify-center overflow-hidden rounded-[var(--radius-xl)] border border-[var(--color-border-strong)] bg-[var(--color-surface-subtle)]/75">
            <div className="text-center">
              ...
              {previewType === "pdf" ? <><p className="mt-2 text-sm font-semibold text-[var(--color-text-primary)]">PDF file detected</p><p className="text-xs text-[var(--color-text-secondary)]">Secure preview is unavailable because no connected file URL is present for this record.</p></> : null}
              {previewType === "image" ? <><p className="mt-2 text-sm font-semibold text-[var(--color-text-primary)]">Image file detected</p><p className="text-xs text-[var(--color-text-secondary)]">Preview is unavailable because no connected file URL is present for this record.</p></> : null}
              {previewType === "unsupported" ? <><p className="mt-2 text-sm font-semibold text-[var(--color-text-primary)]">Unsupported file type</p><p className="text-xs text-[var(--color-text-secondary)]">This format cannot be previewed in the workspace.</p></> : null}
            </div>
          </div>
        )}
        ...
        <div className="flex flex-wrap items-center justify-between gap-3">
          <p className="text-xs font-semibold uppercase tracking-[0.08em] text-[var(--color-text-muted)]">Revision history</p>
          <Button type="button" size="sm" variant="outline" onClick={() => onUploadRevision(selectedDocument)}>Upload revision</Button>
        </div>
        <div><div className="mt-2"><RevisionHistory revisions={selectedDocument.revisionHistory} /></div></div>

        <div>
          <p className="text-xs font-semibold uppercase tracking-[0.08em] text-[var(--color-text-muted)]">Preview status</p>
          <div className="mt-2 rounded-[var(--radius-lg)] bg-[var(--color-surface-subtle)] px-3 py-3 text-sm text-[var(--color-text-secondary)]">
            {previewType === "unsupported" ? "Only image and PDF files are currently supported for preview workflows." : selectedDocument.fileUrl ? "Secure preview is active. The signed file link expires automatically." : "Plan preview is unavailable because no project plan file is connected."}
          </div>
          <div className="mt-3">
            <button type="button" disabled={!selectedDocument.fileUrl} onClick={() => selectedDocument.fileUrl && window.open(selectedDocument.fileUrl, "_blank", "noopener,noreferrer")} className="inline-flex items-center gap-2 rounded-[var(--radius-md)] border border-[var(--color-border-subtle)] bg-[var(--color-surface-card)] px-3 py-2 text-xs font-semibold text-[var(--color-text-secondary)] disabled:cursor-not-allowed disabled:opacity-45">
