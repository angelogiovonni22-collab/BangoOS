    ...
    <Card as="section" variant="elevated">
      <CardHeader>
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
            <CardTitle>Home Solicitation Review</CardTitle>
            <p className="mt-1 text-sm text-[var(--color-text-secondary)]">Determines whether Ohio&apos;s three-business-day cancellation workflow applies to this customer transaction.</p>
          </div>
          <StatusBadge status={status} applicable={evaluation?.applicable ?? null} />
        </div>
      </CardHeader>
      <CardContent className="space-y-5">
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
          <Field label="Personal/family/household purpose">
            <select className={inputClass} value={profile.consumerPurpose} onChange={(e) => setProfile({ ...profile, consumerPurpose: e.target.value as HomeSolicitationProfile["consumerPurpose"] })}>
              <option value="unknown">Select</option><option value="yes">Yes</option><option value="no">No</option>
            </select>
          </Field>
          <Field label="Where was agreement/solicitation made?">
            <select className={inputClass} value={profile.solicitationLocation} onChange={(e) => setProfile({ ...profile, solicitationLocation: e.target.value as HomeSolicitationProfile["solicitationLocation"] })}>
              <option value="unknown">Select</option><option value="buyer_residence">Buyer residence</option><option value="seller_place_of_business">Seller place of business</option><option value="other_away_from_business">Other location away from seller business</option><option value="remote">Remote / electronic</option>
            </select>
          </Field>
          <Field label="Who initiated contact?">
            <select className={inputClass} value={tri(profile.buyerInitiatedContact)} onChange={(e) => setProfile({ ...profile, buyerInitiatedContact: fromTri(e.target.value) })}>
              <option value="unknown">Unknown</option><option value="yes">Buyer</option><option value="no">Seller</option>
            </select>
          </Field>
          <Field label="Fixed Ohio business location?">
            <select className={inputClass} value={tri(profile.sellerHasFixedOhioBusiness)} onChange={(e) => setProfile({ ...profile, sellerHasFixedOhioBusiness: fromTri(e.target.value) })}>
              <option value="unknown">Unknown</option><option value="yes">Yes</option><option value="no">No</option>
            </select>
          </Field>
          <Field label="Separate federal rescission right applies?">
            <select className={inputClass} value={tri(profile.federalRescissionRightApplies)} onChange={(e) => setProfile({ ...profile, federalRescissionRightApplies: fromTri(e.target.value) })}>
              <option value="unknown">Unknown</option><option value="yes">Yes</option><option value="no">No</option>
            </select>
          </Field>
        </div>
        ...
        <div className="grid gap-3 md:grid-cols-2">
          ...
          <Check label="Buyer will receive a seller-signed copy of the agreement" checked={profile.signedSellerCopyConfigured === true} onChange={(v) => setProfile({ ...profile, signedSellerCopyConfigured: v })} />
          ...
          <Check label="Oral cancellation-right disclosure workflow is confirmed at signing" checked={profile.oralDisclosureWorkflowConfirmed === true} onChange={(v) => setProfile({ ...profile, oralDisclosureWorkflowConfirmed: v })} />
        ...
        {profile.cancellationDeadlineDate ? (
          <div className="rounded-[var(--radius-control)] border border-blue-200 bg-blue-50 px-4 py-3 text-sm text-blue-950">
            Cancellation deadline: <strong>{profile.cancellationDeadlineDate}</strong>. Work remains on compliance hold until the applicable cancellation period expires or the record is otherwise lawfully released.
          </div>
        ) : null}
        ...
        {attention.length ? (
          ...
          <div className="rounded-[var(--radius-control)] border border-emerald-300 bg-emerald-50 px-4 py-3 text-sm text-emerald-950">No unresolved automated home-solicitation checks.</div>
        ) : null}

        <div className="flex flex-wrap items-center gap-3">
          <Button type="button" size="md" isLoading={busy} onClick={() => void save()}>Save &amp; Review</Button>
...
function StatusBadge({ status, applicable }: { status: "COMPLIANT" | "ACTION_REQUIRED" | "REVIEW_REQUIRED"; applicable: boolean | null }) {
  ...
  const label = status === "COMPLIANT" ? (applicable === false ? "Not Applicable" : "Ready") : status === "ACTION_REQUIRED" ? "Action Required" : "Review Required";
