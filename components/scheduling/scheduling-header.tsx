      ...
      <div className="flex flex-col gap-6">
        ...
        <div className="grid gap-3 lg:grid-cols-2 xl:grid-cols-4 2xl:grid-cols-6">
          ...
          <label className="space-y-1.5 text-sm font-semibold text-[var(--color-text-primary)]">
            ...
            <Select value={filters.crew} onChange={(event) => onFilterChange("crew", event.target.value)}><option value="all">{t("navigation.allTeams")}</option>{crewOptions.map((option) => <option key={option.id} value={option.id}>{option.name}</option>)}</Select>
          </label>
          ...
          <label className="space-y-1.5 text-sm font-semibold text-[var(--color-text-primary)]">
            ...
            <Select value={filters.groupBy} onChange={(event) => onFilterChange("groupBy", event.target.value as ScheduleFilterState["groupBy"])}><option value="project">{projectLabel}</option><option value="crew">{t("navigation.team")}</option><option value="employee">{t("navigation.teamMember")}</option><option value="trade">{t("scheduling.group.trade")}</option><option value="location">{t("scheduling.group.location")}</option></Select>
          </label>
        </div>
      </div>
    </section>
