  ...
  return (
    ...
    >
      <div data-bos-empty-state="true" className="relative w-full max-w-lg overflow-hidden rounded-[var(--radius-2xl)] border border-[var(--bos-border-subtle)] bg-[var(--bos-bg-panel)] px-[var(--space-6)] py-7 text-center shadow-[var(--bos-card-shadow)]">
        ...
        {icon ? (
          <div className="relative mx-auto flex h-16 w-16 items-center justify-center rounded-full border border-[var(--bos-border-default)] bg-[var(--bos-bg-panel-elevated)] text-2xl font-bold text-[var(--orion-cyan)] shadow-[0_12px_30px_-18px_rgb(37_99_235/0.5)]">
            {icon}
          </div>
        ) : null}

        <h3 className="relative mt-4 text-h3 font-bold text-[var(--bos-text-primary)]">{title}</h3>

        <p className="relative mt-2.5 text-body font-medium text-[var(--bos-text-secondary)]">{description}</p>

        {action || secondaryAction ? (
          <div className="relative mt-6 flex flex-wrap items-center justify-center gap-[var(--space-action-gap)]">
            {secondaryAction}
            {action}
          </div>
        ) : null}
