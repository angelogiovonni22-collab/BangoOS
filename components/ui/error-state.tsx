  ...
  return (
    ...
    >
      <div data-bos-error-state="true" className="relative w-full max-w-lg overflow-hidden rounded-[var(--radius-2xl)] border border-[var(--bos-border-subtle)] bg-[var(--bos-bg-panel)] px-[var(--space-6)] py-[var(--space-8)] text-center shadow-[var(--bos-card-shadow)]">
        ...
        <div className="relative mx-auto flex h-16 w-16 items-center justify-center rounded-full border border-[var(--color-danger-200)] bg-[var(--color-danger-50)] text-2xl font-bold text-[var(--color-danger-700)] shadow-[0_12px_28px_-18px_rgb(220_38_38/0.48)]">
          !
        </div>

        <h3 className="relative mt-5 text-h3 font-semibold text-[var(--bos-text-primary)]">{title}</h3>

        <p className="relative mt-3 text-body font-medium text-[var(--bos-text-secondary)]">{description}</p>
