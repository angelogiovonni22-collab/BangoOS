        ...
        <div className={["fixed inset-0 flex items-center justify-center p-4 pointer-events-none", className || ""].filter(Boolean).join(" ")}>
          <FadeIn durationMs={180} distancePx={4} className="w-full max-w-full">
            <div
              ...
              tabIndex={-1}
              className={[
                "pointer-events-auto mx-auto w-full rounded-[var(--radius-2xl)] border border-[var(--bos-border-default)] bg-white bg-[var(--bos-bg-workspace-card)] p-6 text-[var(--bos-text-primary)] shadow-[var(--shadow-large)]",
                panelClassName || "",
              ].filter(Boolean).join(" ")}
            >
              {title || description ? (
