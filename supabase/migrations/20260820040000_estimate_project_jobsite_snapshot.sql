-- Preserve the address/contact snapshot entered on the estimate when an accepted
-- prospect strongly matches an existing Customer. Customer master data is not
-- overwritten; the generated Project represents the actual quoted job site.
...
begin
  ...
  -- Reload the estimate prospect even after a strong Customer match. This snapshot
  -- is the quoted job/contact context and must win for the generated Project.
  select * into v_prospect
  from public.estimate_prospects
  where company_id = p_company_id and estimate_id = p_estimate_id;
  ...
  insert into public.projects(
    ...
    p_actor_profile_id,
    coalesce(nullif(btrim(coalesce(v_prospect.address_line_1, '')), ''), v_customer.address_line_1),
    coalesce(nullif(btrim(coalesce(v_prospect.address_line_2, '')), ''), v_customer.address_line_2),
    coalesce(nullif(btrim(coalesce(v_prospect.city, '')), ''), v_customer.city),
    coalesce(nullif(btrim(coalesce(v_prospect.state, '')), ''), v_customer.state),
    coalesce(nullif(btrim(coalesce(v_prospect.postal_code, '')), ''), v_customer.postal_code),
    coalesce(nullif(btrim(concat_ws(' ', v_prospect.first_name, v_prospect.last_name)), ''), concat_ws(' ', v_customer.first_name, v_customer.last_name)),
    coalesce(nullif(btrim(coalesce(v_prospect.phone, '')), ''), v_customer.phone),
    coalesce(nullif(btrim(coalesce(v_prospect.email, '')), ''), v_customer.email)
