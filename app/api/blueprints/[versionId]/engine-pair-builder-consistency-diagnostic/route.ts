...
export async function GET(request: Request, { params }: { params: Promise<{ versionId: string }> }) {
  try {
    ...
    if (versionResponse.data.mime_type !== "application/pdf") {
      return NextResponse.json({ error: "Pair-builder consistency diagnostics require a PDF Blueprint." }, { status: 415 });
    }
    const fileSize = Number(versionResponse.data.file_size_bytes || 0);
    if (fileSize <= 0 || fileSize > MAX_SOURCE_BYTES) {
      return NextResponse.json({ error: "Blueprint source is outside the safe read-only diagnostic size limit." }, { status: 413 });
    }
    ...
    if (!selected.scale.drawingUnitsPerMeter || selected.scale.confidence < 0.55) {
      throw new Error("Verified Blueprint scale is required for pair-builder consistency diagnostics.");
    }
    ...
    const pairingGap = diagnoseSourcePairRasterPairingGaps({
      familyAgreement,
      consolidationClusters: sourceEvidence.consolidated.clusters,
    ...
    const stageGap = diagnoseNoMatchingRasterStages({
      consolidationClusters: sourceEvidence.consolidated.clusters,
      rasterPairingGapDiagnostic: pairingGap,
      rawSegments: candidate.rawSegments,
      annotationFilteredSegments: candidate.annotationFiltered,
      ...shared,
    });
    const diagnostic = diagnosePairBuilderConsistencyGaps({
      stageDiagnostic: stageGap,
      consolidationClusters: sourceEvidence.consolidated.clusters,
      annotationFilteredSegments: candidate.annotationFiltered,
      explicitWallSystems: candidate.explicitSystems,
      ...shared,
    ...
    return NextResponse.json({
      mode: "read_only_pair_builder_consistency_diagnostic",
      source: {
        versionId,
        sheetNumber: String(sheetResponse.data.sheet_number || ""),
      ...
      extraction: {
        ...
        sourcePairCount: sourceEvidence.network.wallFacePairs.length,
      },
      familyAgreement: {
      ...
      stageGap: {
        reasonMemberCounts: stageGap.reasonMemberCounts,
        reasonFamilyCounts: stageGap.reasonFamilyCounts,
      },
      diagnostic,
      safety: {
    ...
    return NextResponse.json({
      error: error instanceof Error ? error.message : "Unable to run pair-builder consistency diagnostics.",
    }, { status: 400 });
  }
}
