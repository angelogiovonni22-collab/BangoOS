...
import { diagnoseUnexpectedPostRunGaps } from "@/lib/blueprints/engine/source-pair-raster-unexpected-post-run-gap-diagnostic";
...
export async function GET(request: Request, { params }: { params: Promise<{ versionId: string }> }) {
  try {
    ...
    if (versionResponse.data.mime_type !== "application/pdf") {
      return NextResponse.json({ error: "Unexpected post-run gap diagnostics require a PDF Blueprint." }, { status: 415 });
    }
    ...
    if (!selected.scale.drawingUnitsPerMeter || selected.scale.confidence < 0.55) {
      throw new Error("Verified Blueprint scale is required for unexpected post-run gap diagnostics.");
    }
    ...
    const diagnostic = diagnoseUnexpectedPostRunGaps({
    ...
    return NextResponse.json({
      mode: "read_only_unexpected_post_run_gap_diagnostic",
      source: {
        versionId,
        sheetNumber: String(sheetResponse.data.sheet_number || ""),
    ...
    return NextResponse.json({
      error: error instanceof Error ? error.message : "Unable to run unexpected post-run Blueprint gap diagnostics.",
    }, { status: 400 });
  }
}
