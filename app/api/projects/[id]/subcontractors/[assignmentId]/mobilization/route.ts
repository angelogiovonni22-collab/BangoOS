...
async function refreshMobilization(admin: ReturnType<typeof createAdminClient>, companyId: string, assignmentId: string) {
  const result = await admin.rpc("refresh_subcontractor_mobilization_status" as never, {
    p_company_id: companyId,
    p_assignment_id: assignmentId,
  } as never) as unknown as MobilizationRefreshResult;
  if (result.error) throw new Error(result.error.message || "Unable to refresh mobilization status.");
...
export async function GET(_request: Request, { params }: { params: Promise<{ id: string; assignmentId: string }> }) {
  try {
    ...
    const [{ data: authorization }, { data: master }, { data: requirements }, refreshed] = await Promise.all([
      ...
      refreshMobilization(admin, workspace.companyId, assignmentId),
    ]);
    return NextResponse.json({ assignmentContractStatus: assignment.contract_status, authorization, master, requirements: requirements || [], mobilizationStatus: refreshed?.mobilization_status || "not_cleared", blockers: refreshed?.blockers || [] });
  } catch (error) {
