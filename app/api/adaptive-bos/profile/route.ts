...
export async function GET() {
  ...
  const profile:AdaptiveBosCompanyProfile = data ? {
  ...
  return NextResponse.json({ ok:true, profile:data, resolved:resolveAdaptiveBosConfig(profile) });
}
...
export async function PATCH(req:Request) {
  ...
  return NextResponse.json({ ok:true, profile:data, resolved:resolveAdaptiveBosConfig({ industryKey:data.industry_key, industryLabel:data.industry_label, businessModel:data.business_model, primaryServices:data.primary_services, moduleOverrides:data.module_overrides, terminologyOverrides:data.terminology_overrides, workflowOverrides:data.workflow_overrides }) });
}
