...
async function resolveRequestContext(req: NextRequest) {
  ...
  if (!IS_PRODUCTION && typeof console !== "undefined") {
    console.info("[orion-timing] context.lookup.start");
  }
...
export async function GET(req: NextRequest): Promise<NextResponse> {
  try {
    ...
    if (requestedUrl.searchParams.get("mode") === "related" && requestedUrl.searchParams.get("entityType") === "customer") {
      ...
      if (!customerId) {
        return NextResponse.json({ ok: false, error: "customerId is required for customer related records." }, { status: 400 });
      }
      ...
      try {
        ...
        return NextResponse.json(
          { ok: false, error: error instanceof Error ? error.message : "Unable to load related records." },
          { status: 400 },
        );
      }
    }
    ...
    return NextResponse.json(
      { ok: false, error: error instanceof Error ? error.message : "Unable to load command center catalog.", statusCategory: "error" },
      { status: 500 },
    );
  }
}
...
export async function POST(req: NextRequest): Promise<NextResponse> {
  try {
    ...
    if (!body || typeof body !== "object") {
      return NextResponse.json({ ok: false, error: "Invalid command payload." }, { status: 400 });
    }
    ...
    if (payload.mode === "intent") {
      ...
      if (!IS_PRODUCTION && typeof console !== "undefined") {
        console.info("[orion-timing] intent.request.start");
      }
    ...
    if (!IS_PRODUCTION && typeof console !== "undefined") {
      console.info("[orion-timing] command.validation.start", { commandId });
    }
    const validation = command.validate(inputParams);
    logApiTiming("command.validation.end", validationStartedAt, {
      ok: validation.ok,
      commandId,
    });
    if (!validation.ok) {
  ...
  const executionStartedAt = nowMs();
    ...
    const result = await router.executeCommand({
      ...
      userContext: {
        actorProfileId: context.workspace.userId,
        role: normalizeRole(context.workspace.role),
      },
      executionContext: {
        origin: "user",
      },
    ...
    logApiTiming("command.execute.end", executionStartedAt, {
      commandId,
    ...
    return NextResponse.json(
      { ok: false, error: error instanceof Error ? error.message : "Command execution request failed.", statusCategory: "command_execution_failed" },
      { status: 500 },
    );
  }
}
