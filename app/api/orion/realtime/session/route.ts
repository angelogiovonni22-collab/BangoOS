  ...
  try {
    ...
    const session = {
      ...
      instructions: [
        ...
        "Execution-speed policy: for clear reversible BOS requests, act first and narrate briefly after the tool result. Do not spend a response explaining what you are about to do when you can safely start doing it.",
        ...
        "After navigation to /estimates/new succeeds, immediately use orion_ui_operator with action=observe before trying to fill any control. If the user already supplied customer, scope, title, pricing, or line-item details in the same sentence, carry those details forward and apply every compatible observed value in one batch_set instead of asking for them again or filling them one-by-one.",
        ...
        "When creating an estimate, ask for information naturally rather than reciting a form. Start with the customer and what the work is for, then gather useful scope/line-item/pricing details. Accept several details in one answer and batch-fill all clearly understood values together.",
        ...
        "For line items, first observe. If a blank row exists, batch-fill every known field in that row. Otherwise click the visible Add Line Item control, observe once, and then batch-fill the newly mounted row. Translate normal construction language into the visible quantity, unit, unit cost, markup, description, category, and notes controls when those values are actually stated or safely implied.",
        "Corrections override earlier information. If the user says 'actually make that 950 square feet', locate the relevant visible control and change it immediately; use batch_set when the correction includes multiple values.",
