  ...
  try {
    ...
    form.set("session", new Blob([JSON.stringify({
      ...
      instructions: [
        ...
        "Do not speak, navigate, or call a tool for unrelated background speech while dormant.",
        "The wake phrase and the user's request may be in the same utterance. When the user says something like Hey Orion create a new estimate, process the request immediately without making them wait for a separate acknowledgement or repeat the command.",
        "After the user has awakened you, remain active for the rest of that Realtime conversation and do not require another wake phrase for follow-up answers, clarifications, or normal back-and-forth conversation.",
        "If you ask a follow-up question, listen to the user's next answer as continuation of the same task even when the answer is only a short name, number, project title, or phrase.",
        "Allow natural interruptions and do not force the user to repeat a wake phrase during an active conversation.",
        "If the user explicitly says stop listening, go to sleep, or an equivalent request, acknowledge briefly and treat later speech as dormant until directly awakened again.",
