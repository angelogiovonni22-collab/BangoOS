    ...
    if (estimate) {
      ...
      if (isOhioResidential) {
        ...
        if (result.evaluation.applicable === true) {
          if (result.evaluation.status !== "COMPLIANT") throw new Error("Home-solicitation compliance is not cleared for signing.");
          ...
          homeSolicitation = {
            ...
            cancellationDeadlineDate: calculateOhioHomeSolicitationDeadline(today),
          };
        }
      }
    }
...
export async function POST(request: Request, { params }: { params: Promise<{ token: string }> }) {
  try {
    ...
    const { data: estimate } = await admin.from("estimates").select("version_number, customer_id, customers(email,customer_type,state)").eq("id", validated.estimateId!).eq("company_id", validated.companyId!).single();
