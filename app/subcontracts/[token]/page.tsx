...
type MasterSection = { title: string; body: string };
type MasterSnapshot = { sections?: MasterSection[] };
...
type ContractPayload = {
  ...
  master: { status: string; version: string; snapshot: MasterSnapshot; hash: string; signedAt: string | null };
  authorization: { status: string; version: string; snapshot: WorkAuthorizationSnapshot; hash: string; signedAt: string | null };
  expiresAt: string;
};

const money = (value: number | null) => value == null ? "Not specified" : Number(value).toLocaleString(undefined, { style: "currency", currency: "USD" });
...
export default function SubcontractSigningPage({ params }: { params: Promise<{ token: string }> }) {
  ...
  useEffect(() => {
    void params.then(({ token: value }) => {
      ...
      fetch(`/api/subcontracts/${encodeURIComponent(value)}`)
        .then(async (response) => {
          const body = await response.json() as ContractPayload & { error?: string };
  ...
  async function sign() {
    ...
    try {
      ...
      const body = await response.json() as { error?: string; mobilizationStatus?: string; blockers?: string[] };
  ...
  const masterSections = data.master.snapshot.sections ?? [];
