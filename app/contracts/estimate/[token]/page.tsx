...
type Contract = {
  company: { name: string };
  estimate: { title: string; estimate_number: string | null; description: string | null; total_amount: number; terms: string | null; payment_terms: string | null; scope_inclusions: string | null; scope_exclusions: string | null; status: string };
...
export default function EstimateContractPage({ params }: { params: Promise<{ token: string }> }) {
  ...
  async function cancelTransaction() {
    if (!window.confirm("Cancel this transaction? B.O.S. will immediately record your written cancellation notice.")) return;
    setCancelling(true); setCancellationMessage(null);
    try {
      const response = await fetch(`/api/contracts/estimate/${encodeURIComponent(token)}/cancel`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ confirmation: true, notice: "I hereby cancel this transaction." }) });
      const body = await response.json();
      if (!response.ok) throw new Error(body.error || "Unable to record cancellation.");
      if (body.cancelled) {
        setCancelled(true);
        setCancellationMessage("Your cancellation notice has been recorded by B.O.S.");
      } else {
        setCancellationMessage(body.message || "Your cancellation request has been recorded for review.");
      }
    } catch (caught) { setCancellationMessage(caught instanceof Error ? caught.message : "Unable to record cancellation."); }
  ...
  if (cancelled) return <main className="mx-auto max-w-3xl p-6"><div className="rounded-3xl border border-blue-200 bg-blue-50 p-8"><h1 className="text-2xl font-bold text-blue-950">Transaction cancelled</h1><p className="mt-3 text-blue-800">B.O.S. has recorded the cancellation notice. Bango Construction has been notified and the related project is on compliance hold.</p>{cancellationMessage ? <p className="mt-3 font-semibold text-blue-950">{cancellationMessage}</p> : null}</div></main>;
  if (signed || persistentSigned) return <main className="mx-auto max-w-3xl space-y-5 p-6"><div className="rounded-3xl border border-emerald-200 bg-emerald-50 p-8"><h1 className="text-2xl font-bold text-emerald-950">Estimate signed</h1><p className="mt-3 text-emerald-800">Your estimate has been accepted and B.O.S. has created the project.</p>{deadline ? <p className="mt-3 font-semibold text-emerald-950">Your Ohio cancellation period runs through midnight on {deadline}. Bango Construction will not begin covered services during that period.</p> : null}</div>{contract.homeSolicitation ? <div className="rounded-3xl border border-slate-300 bg-white p-6"><h2 className="text-xl font-bold text-slate-950">Need to cancel?</h2><p className="mt-2 text-sm leading-6 text-slate-700">You may use the cancellation notice provided with this agreement, send another written notice using the seller contact information, or record your written cancellation through this secure B.O.S. link.</p><button type="button" className="mt-4 rounded-xl border border-red-300 bg-red-50 px-5 py-3 font-semibold text-red-800 disabled:opacity-50" disabled={cancelling} onClick={() => void cancelTransaction()}>{cancelling ? "Recording cancellation…" : "Cancel Transaction"}</button>{cancellationMessage ? <p className="mt-3 text-sm font-semibold text-slate-800">{cancellationMessage}</p> : null}</div> : null}</main>;
  ...
  return <main className="mx-auto max-w-4xl space-y-6 p-4 text-slate-950 sm:p-8">
    ...
    <section className="rounded-3xl border border-blue-200 bg-blue-50 p-6 text-slate-950"><h2 className="text-xl font-semibold text-slate-950">Digital signature</h2>{contract.homeSolicitation ? <p className="mt-4 rounded-xl border-2 border-slate-900 bg-white p-4 text-sm font-bold leading-6 text-slate-950">You, the buyer, may cancel this transaction at any time prior to midnight of the third business day after the date of this transaction. See the attached notice of cancellation for an explanation of this right.</p> : null}<label className="mt-4 block text-sm font-semibold text-slate-900" htmlFor="legal-name">Full legal name</label><input id="legal-name" className="mt-1 w-full rounded-xl border border-slate-300 bg-white px-4 py-3 text-slate-950 placeholder:text-slate-500" value={typedName} onChange={(event) => setTypedName(event.target.value)} autoComplete="name" /><label className="mt-4 flex gap-3 text-sm text-slate-800"><input type="checkbox" checked={consent} onChange={(event) => setConsent(event.target.checked)} /><span>By signing, I confirm that I have reviewed and accept this estimate, its scope, price, project-specific terms, payment terms, and the incorporated Construction Agreement. I consent to use my typed name as my electronic signature and represent that I am authorized to accept for the customer. {contract.homeSolicitation ? "I also acknowledge that I received the Ohio cancellation-right notices shown with this agreement. " : ""}I also agree to the <a className="font-semibold text-blue-800 underline underline-offset-2" href="/legal/electronic-signature-and-platform-terms" target="_blank" rel="noreferrer">B.O.S. Electronic Signature &amp; Platform Terms</a>.</span></label>{contract.homeSolicitation ? <div className="mt-5 rounded-xl border border-slate-300 bg-white p-4 text-sm text-slate-700"><p className="font-semibold text-slate-950">Seller acceptance</p><p className="mt-1">{contract.homeSolicitation.sellerName}</p><p>Authorized signer: {contract.homeSolicitation.sellerSignerName}</p><p>Electronically signed: {contract.homeSolicitation.sellerSignedAt ? new Date(contract.homeSolicitation.sellerSignedAt).toLocaleString() : "Not recorded"}</p></div> : null}{error ? <p className="mt-3 text-sm text-red-700">{error}</p> : null}<button className="mt-5 rounded-xl bg-blue-700 px-5 py-3 font-semibold text-white disabled:opacity-50" disabled={submitting || !typedName.trim() || !consent} onClick={() => void sign()}>{submitting ? "Signing…" : "Sign estimate"}</button></section>
  </main>;
}

function HomeSolicitationNotices({ notice }: { notice: HomeSolicitationNotice }) {
  return <section className="rounded-3xl border-2 border-slate-900 bg-white p-6 text-slate-950"><div className="flex flex-wrap items-start justify-between gap-3"><div><p className="text-xs font-bold uppercase tracking-[0.16em] text-slate-500">Ohio consumer notice</p><h2 className="mt-1 text-2xl font-bold">Right to Cancel</h2></div><p className="text-sm font-semibold">Deadline: midnight {notice.cancellationDeadlineDate}</p></div><div className="mt-4 grid gap-2 rounded-xl bg-slate-50 p-4 text-sm sm:grid-cols-2"><p><strong>Transaction date:</strong> {notice.transactionDate}</p><p><strong>Seller:</strong> {notice.sellerName}</p><p><strong>Seller address:</strong> {notice.sellerAddress}</p><p><strong>Cancellation email:</strong> {notice.cancellationEmail || "Not provided"}</p>{notice.cancellationFax ? <p><strong>Cancellation fax:</strong> {notice.cancellationFax}</p> : null}<p><strong>Seller signer:</strong> {notice.sellerSignerName}</p></div><div className="mt-6 grid gap-4 md:grid-cols-2"><CancellationCopy notice={notice} copy="Copy 1" /><CancellationCopy notice={notice} copy="Copy 2" /></div></section>;
}
