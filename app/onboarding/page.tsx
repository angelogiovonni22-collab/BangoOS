...
export default function OnboardingPage() {
  ...
  async function handleFinish() {
    ...
    try {
      ...
      const baseCompanyPayload = {
        ...
        contractor_license: trimToNull(form.contractorLicense),
        insurance_provider: trimToNull(form.insuranceProvider),
        years_in_business: yearsInBusiness,
        owner_name: trimToNull(form.ownerName),
      ...
      const { error: completionError } = await supabase
        ...
        .eq("id", companyId);
      if (completionError) return setError(t("onboarding.errorWorkspaceUpdate", { message: completionError.message }));
  ...
  if (checkingRole) {
    return (
      <main className="min-h-screen bg-[var(--color-surface-app)] px-6 py-12 text-[var(--color-text-primary)]">
      ...
      <div className="mx-auto max-w-5xl">
        <div className="overflow-hidden rounded-[var(--radius-3xl)] border border-[var(--color-border-subtle)] bg-[var(--color-surface-card)] shadow-[var(--shadow-large)]">
          ...
          <section className="px-8 py-8 sm:px-12 sm:py-10">
            <div className="mb-10">
              <div className="flex items-center justify-between gap-4 text-sm font-semibold">
                ...
                <span className="text-right text-[var(--color-text-secondary)]">
                  {step === 1 ? t("onboarding.stepCompany") : step === 2 ? t("onboarding.stepBusiness") : step === 3 ? t("onboarding.stepOwner") : t("onboarding.stepReview")}
                </span>
              </div>
              <div className="mt-3 h-2 overflow-hidden rounded-full bg-[var(--color-surface-muted)]">
                <div className="h-full rounded-full bg-[var(--color-brand-600)] transition-all duration-300" style={{ width: progressWidth }} />
              </div>
            </div>

            {step === 1 ? (
              <div className="space-y-6">
                <Section title={t("onboarding.companyInfoTitle")} description={t("onboarding.companyInfoDescription")} />
            ...
            {step === 2 ? (
              <div className="space-y-6">
                <Section title={t("onboarding.businessDetailsTitle")} description={t("onboarding.businessDetailsDescription")} />
                ...
                <Field label={t("onboarding.insuranceProvider")} value={form.insuranceProvider} placeholder={t("onboarding.insuranceProviderPlaceholder")} onChange={(value) => updateField("insuranceProvider", value)} />
                ...
                <div className="rounded-xl border border-[var(--color-border-subtle)] bg-[var(--color-surface-subtle)] px-4 py-3 text-sm text-[var(--color-text-secondary)]">{t("onboarding.taxAutomaticNotice")}</div>
              </div>
            ) : null}

            {step === 3 ? (
              <div className="space-y-6">
                <Section title={t("onboarding.ownerTypeTitle")} description={t("onboarding.ownerTypeDescription")} />
            ...
            {step === 4 ? (
              <div className="space-y-5">
                <Section title={t("onboarding.reviewTitle")} description={t("onboarding.reviewDescription")} />
                ...
                <ReviewRow label={t("onboarding.taxSetup")} value={t("onboarding.taxAutomaticValue")} />
              </div>
            ) : null}

            {error ? (
              <div role="alert" className="mt-6 rounded-[var(--radius-lg)] border border-[var(--color-danger-200)] bg-[var(--color-danger-50)] px-4 py-3 text-sm font-medium text-[var(--color-danger-700)]">{error}</div>
            ) : null}

            <div className="mt-8 flex flex-col-reverse gap-3 sm:flex-row">
              {step > 1 ? <Button type="button" onClick={handleBack} disabled={isSaving} variant="outline" size="lg" fullWidth>{t("onboarding.back")}</Button> : null}
              {step < 4 ? <Button type="button" onClick={handleNext} size="lg" fullWidth>{t("onboarding.continue")}</Button> : <Button type="button" onClick={handleFinish} disabled={isSaving} size="lg" fullWidth>{isSaving ? t("onboarding.creatingWorkspace") : t("onboarding.createWorkspace")}</Button>}
            </div>
          </section>
        </div>
      </div>
...
function Field({ label, value, placeholder, helperText, type = "text", required = false, min, onChange }: { label: string; value: string; placeholder?: string; helperText?: string; type?: string; required?: boolean; min?: string; onChange: (value: string) => void }) {
  const id = label.toLowerCase().replace(/[^a-z0-9]+/g, "-");
  return (
    <div className="space-y-2">
      <FormLabel htmlFor={id} required={required}>{label}</FormLabel>
      <Input id={id} type={type} value={value} onChange={(event) => onChange(event.target.value)} placeholder={placeholder} required={required} min={min} />
      {helperText ? <p className="text-sm text-[var(--color-text-secondary)]">{helperText}</p> : null}
    </div>
  );
}

function BusinessTypeButton({ label, description, selected, onClick }: { label: string; description: string; selected: boolean; onClick: () => void }) {
  return (
    <button type="button" onClick={onClick} aria-pressed={selected} className={`rounded-[var(--radius-xl)] border px-4 py-4 text-left transition ${selected ? "border-[var(--color-brand-600)] bg-[#18355f] text-white ring-2 ring-[var(--focus-ring-primary)]" : "border-[var(--color-border-subtle)] bg-[var(--color-surface-subtle)] text-[var(--color-text-primary)] hover:border-[var(--color-border-strong)] hover:bg-[var(--color-surface-muted)]"}`}>
      <span className="block font-bold text-current">{label}</span>
      <span className={`mt-1 block text-sm ${selected ? "text-blue-100" : "text-[var(--color-text-secondary)]"}`}>{description}</span>
    </button>
  );
}
...
function ReviewRow({ label, value }: { label: string; value: string }) {
  ...
  return (
    <div className="flex flex-col gap-1 rounded-[var(--radius-xl)] border border-[var(--color-border-subtle)] bg-[var(--color-surface-subtle)] px-4 py-4 shadow-[var(--shadow-small)] sm:flex-row sm:items-center sm:justify-between sm:gap-6">
      ...
      <span className="break-words font-semibold text-[var(--color-text-primary)] sm:text-right">{value || t("onboarding.notProvided")}</span>
    </div>
  );
}

function formatBusinessType(type: BusinessType, t: (key: string) => string) {
  if (type === "residential") return t("onboarding.residential");
  if (type === "commercial") return t("onboarding.commercial");
