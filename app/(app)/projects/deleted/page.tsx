...
export default function DeletedProjectsPage() {
  ...
  async function restoreProject(project: DeletedProject) {
    ...
    try {
      const response = await fetch(`/api/projects/${project.projectId}/lifecycle`, {
        method: "PATCH",
  ...
  return (
    <div className="container-content space-y-[var(--space-section)]">
      <PageHeader
        ...
        description="Review projects removed from the active list and restore them without losing their project history."
        primaryAction={<Link href="/projects"><Button variant="outline">Back to Projects</Button></Link>}
      />

      <div className="flex flex-wrap items-center gap-2" aria-label="Project lifecycle views">
        <Link href="/projects"><Button type="button" size="sm" variant="outline">Projects</Button></Link>
        <Button type="button" size="sm" variant="primary" disabled>Previously Deleted</Button>
      </div>
          ...
          <EnterpriseTable ariaLabel="Previously deleted projects">
            <EnterpriseTableHead>
              <tr>
                ...
                <EnterpriseTableHeading>Previous Status</EnterpriseTableHeading>
                <EnterpriseTableHeading>Deleted</EnterpriseTableHeading>
            ...
            <EnterpriseTableBody>
              {projects.map((project) => (
                <EnterpriseTableRow key={project.historyId}>
                  <EnterpriseTableCell className="font-semibold">{project.projectName}</EnterpriseTableCell>
                  <EnterpriseTableCell>{project.customerName}</EnterpriseTableCell>
                  <EnterpriseTableCell>{formatStatus(project.previousStatus)}</EnterpriseTableCell>
                  <EnterpriseTableCell>{formatDeletedAt(project.deletedAt)}</EnterpriseTableCell>
                  <EnterpriseTableCell align="right">
                    <Button size="sm" variant="outline" disabled={busyId === project.projectId} onClick={() => void restoreProject(project)}>
                      <RotateCcw size={14} aria-hidden="true" />
                      {busyId === project.projectId ? "Restoring…" : "Restore"}
                    </Button>
                  </EnterpriseTableCell>
                </EnterpriseTableRow>
              ))}
