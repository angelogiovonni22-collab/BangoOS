...
type ProfileRow = { id: string; project_id: string; applicability: string; jurisdiction: string; determination_number: string | null; determination_title: string | null; effective_date: string | null; expiration_date: string | null; contracting_agency: string | null; certified_payroll_required: boolean; weekly_statement_required: boolean; wage_posting_required: boolean; completion_affidavit_required: boolean };
...
type ProjectCompliance = { profile: ProfileRow; projectName: string; compliantWorkers: number; deficientWorkers: number; estimatedDeficiency: number; payrollPeriods: number; payrollExceptions: number };
...
export default function PrevailingWagePage() {
  ...
  const load = useCallback(async () => {
    if (!supabase) { setErrorMessage("Unable to connect to B.O.S. compliance right now."); setIsLoading(false); return; }
    ...
    try {
      ...
      const complianceResults = await Promise.all(profiles.filter((row) => row.applicability !== "not_applicable").map(async (profile) => {
        ...
        return { profile, projectName: projectNames.get(profile.project_id) || "Project", compliantWorkers: compliance.compliantWorkerCount, deficientWorkers: compliance.deficientWorkerCount, estimatedDeficiency: compliance.totalEstimatedDeficiency, payrollPeriods: projectPayroll.length, payrollExceptions: projectPayroll.filter((row) => ["rejected", "corrected"].includes(row.status) || (row.status === "submitted" && !row.statement_of_compliance_signed)).length } satisfies ProjectCompliance;
      }));
      ...
      setItems(complianceResults); setCanConfigure(WRITE_ROLES.has((workspace.context.role || "").toLowerCase()));
    } catch (error) { setErrorMessage(error instanceof Error ? error.message : "Unable to load prevailing wage compliance."); }
  ...
  return <div className="container-content space-y-[var(--space-section)]">
    ...
    <section className="space-y-4"><div className="flex flex-wrap items-end justify-between gap-3"><div><p className="text-xs font-bold uppercase tracking-[0.18em] text-[var(--bos-text-muted)]">Project compliance register</p><h2 className="mt-1 text-xl font-semibold">Covered commercial/public projects</h2></div><Link href="/invoices/accounts-payable" className="text-sm font-semibold text-blue-400 hover:text-blue-300">Accounts Payable →</Link></div>
      ...
      {!isLoading && items.length === 0 ? <div className="rounded-2xl border border-[var(--bos-border-default)] bg-[var(--bos-bg-panel)] p-8 text-center"><p className="font-semibold">No covered prevailing-wage projects yet.</p><p className="mt-1 text-sm text-[var(--bos-text-secondary)]">Configure the project’s governing wage determination before field payroll begins.</p>{canConfigure ? <Link href="/invoices/prevailing-wage/setup" className="mt-4 inline-flex text-sm font-semibold text-blue-400">Configure first project →</Link> : null}</div> : null}
      <div className="grid gap-4 xl:grid-cols-2">{items.map((item) => <article key={item.profile.id} className="rounded-2xl border border-[var(--bos-border-default)] bg-[var(--bos-bg-panel)] p-5 shadow-[var(--shadow-card)]"><div className="flex flex-wrap items-start justify-between gap-3"><div><p className="text-xs font-bold uppercase tracking-[0.14em] text-[var(--bos-text-muted)]">{labelApplicability(item.profile.applicability)}</p><h3 className="mt-1 text-lg font-semibold">{item.projectName}</h3><p className="mt-1 text-sm text-[var(--bos-text-secondary)]">{item.profile.determination_title || item.profile.determination_number || item.profile.contracting_agency || "Wage determination configured"}</p></div><ComplianceBadge deficient={item.deficientWorkers > 0 || item.estimatedDeficiency > 0} /></div><div className="mt-5 grid grid-cols-2 gap-3 sm:grid-cols-4"><MiniMetric label="Compliant" value={String(item.compliantWorkers)} /><MiniMetric label="Deficient" value={String(item.deficientWorkers)} danger={item.deficientWorkers > 0} /><MiniMetric label="Deficiency" value={currency(item.estimatedDeficiency)} danger={item.estimatedDeficiency > 0} /><MiniMetric label="Payroll" value={String(item.payrollPeriods)} /></div><div className="mt-5 flex flex-wrap gap-2 text-xs text-[var(--bos-text-secondary)]">{item.profile.certified_payroll_required ? <Flag>Certified payroll</Flag> : null}{item.profile.weekly_statement_required ? <Flag>Weekly statement</Flag> : null}{item.profile.wage_posting_required ? <Flag>Wage posting</Flag> : null}{item.profile.completion_affidavit_required ? <Flag>Completion affidavit</Flag> : null}{item.profile.effective_date ? <Flag>Effective {item.profile.effective_date}</Flag> : null}</div><div className="mt-5 flex flex-wrap gap-4 border-t border-[var(--bos-border-subtle)] pt-4"><Link href={`/invoices/prevailing-wage/${item.profile.project_id}`} className="text-sm font-semibold text-blue-400 hover:text-blue-300">Manage wage & payroll →</Link><Link href={`/projects/${item.profile.project_id}`} className="text-sm font-semibold text-[var(--bos-text-secondary)] hover:text-[var(--bos-text-primary)]">Project workspace</Link></div></article>)}</div>
    </section>
  </div>;
}
