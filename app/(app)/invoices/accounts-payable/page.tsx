...
export default function AccountsPayablePage() {
  ...
  const load = useCallback(async () => {
    if (!supabase) { setErrorMessage("Unable to connect to B.O.S. finance right now."); setIsLoading(false); return; }
    ...
    } catch (error) { setErrorMessage(error instanceof Error ? error.message : "Unable to load accounts payable."); }
  ...
  return <div className="container-content space-y-[var(--space-section)]">
    ...
    <section className="overflow-hidden rounded-2xl border border-[var(--bos-border-default)] bg-[var(--bos-bg-panel)] shadow-[var(--shadow-card)]">
      <div className="flex flex-wrap items-center justify-between gap-3 border-b border-[var(--bos-border-subtle)] px-5 py-4"><div><p className="text-xs font-bold uppercase tracking-[0.18em] text-[var(--bos-text-muted)]">Vendor bill register</p><h2 className="mt-1 text-lg font-semibold">Current AP activity</h2></div><div className="flex gap-4"><Link href="/vendors" className="text-sm font-semibold text-blue-400 hover:text-blue-300">Vendors</Link><Link href="/invoices/prevailing-wage" className="text-sm font-semibold text-blue-400 hover:text-blue-300">Prevailing Wage →</Link></div></div>
      {isLoading ? <p className="p-6 text-sm text-[var(--bos-text-secondary)]">Loading accounts payable…</p> : bills.length === 0 ? <div className="p-8 text-center"><p className="font-semibold">No vendor bills yet.</p><p className="mt-1 text-sm text-[var(--bos-text-secondary)]">The AP foundation is active and ready for vendor bill entry.</p>{canManage ? <Link href="/invoices/accounts-payable/new" className="mt-4 inline-flex text-sm font-semibold text-blue-400">Create first vendor bill →</Link> : null}</div> : <div className="overflow-x-auto"><table className="min-w-full text-left text-sm"><thead className="bg-[var(--bos-bg-control)] text-xs uppercase tracking-[0.12em] text-[var(--bos-text-muted)]"><tr><th className="px-5 py-3">Bill</th><th className="px-5 py-3">Vendor</th><th className="px-5 py-3">Project</th><th className="px-5 py-3">Due</th><th className="px-5 py-3">Status</th><th className="px-5 py-3 text-right">Total</th><th className="px-5 py-3 text-right">Balance</th></tr></thead><tbody className="divide-y divide-[var(--bos-border-subtle)]">{bills.map((bill) => <tr key={bill.id} className="hover:bg-[var(--bos-bg-hover)]"><td className="px-5 py-4"><Link href={`/invoices/accounts-payable/${bill.id}`} className="font-semibold text-blue-400 hover:text-blue-300">{bill.bill_number}</Link><p className="text-xs text-[var(--bos-text-muted)]">{bill.vendor_invoice_number || bill.bill_date}</p></td><td className="px-5 py-4">{vendorNames[bill.vendor_id] || "Vendor"}</td><td className="px-5 py-4">{bill.project_id ? projectNames[bill.project_id] || "Project" : "Company overhead"}</td><td className="px-5 py-4">{bill.due_date || "—"}</td><td className="px-5 py-4"><StatusPill status={bill.status} /></td><td className="px-5 py-4 text-right font-medium">{currency(bill.total_amount)}</td><td className="px-5 py-4 text-right font-semibold">{currency(bill.balance_due)}</td></tr>)}</tbody></table></div>}
    </section>
  </div>;
}
...
function StatusPill({ status }: { status: string }) { return <span className="inline-flex rounded-full border border-[var(--bos-border-default)] bg-[var(--bos-bg-control)] px-2.5 py-1 text-xs font-semibold capitalize">{(status || "draft").replaceAll("_", " ")}</span>; }
