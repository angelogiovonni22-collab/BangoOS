...
export function MeasureClient(){
  ...
  const [reference,setReference]=useState(12); const [verified,setVerified]=useState(0);
  ...
  useEffect(()=>{ if(calculated>0)setVerified(Number(calculated.toFixed(2))); },[calculated]);

  function redraw(){ const canvas=canvasRef.current,img=imageRef.current;if(!canvas||!img)return; const max=900,scale=Math.min(1,max/img.naturalWidth); canvas.width=Math.round(img.naturalWidth*scale);canvas.height=Math.round(img.naturalHeight*scale); const ctx=canvas.getContext("2d");if(!ctx)return;ctx.drawImage(img,0,0,canvas.width,canvas.height); points.forEach((p,i)=>{ctx.beginPath();ctx.arc(p.x,p.y,8,0,Math.PI*2);ctx.fillStyle=i<2?"#f59e0b":"#22c55e";ctx.fill();}); if(points.length>=2){ctx.strokeStyle="#f59e0b";ctx.lineWidth=4;ctx.beginPath();ctx.moveTo(points[0].x,points[0].y);ctx.lineTo(points[1].x,points[1].y);ctx.stroke();} if(points.length>=4){ctx.strokeStyle="#22c55e";ctx.beginPath();ctx.moveTo(points[2].x,points[2].y);ctx.lineTo(points[3].x,points[3].y);ctx.stroke();} }
  useEffect(redraw,[points]);

  function choosePhoto(file:File|null){ setPhoto(file);setPoints([]);setVerified(0); if(!file)return; const img=new Image();img.onload=()=>{imageRef.current=img;redraw()};img.src=URL.createObjectURL(file); }
  function tapCanvas(e:React.MouseEvent<HTMLCanvasElement>){ if(points.length>=4)return; const c=e.currentTarget,r=c.getBoundingClientRect();setPoints(p=>[...p,{x:(e.clientX-r.left)*c.width/r.width,y:(e.clientY-r.top)*c.height/r.height}]); }
  function resetPoints(){setPoints([]);setVerified(0);}
  ...
  return <div className="mx-auto max-w-6xl space-y-6 p-4 md:p-6">
    <header><p className="text-xs font-semibold uppercase tracking-[.18em] text-blue-500">B.O.S. Field Tool</p><h1 className="text-3xl font-bold">B.O.S. Measure</h1><p className="mt-2 text-sm text-muted-foreground">Camera-assisted measurements saved directly to a project or estimate. Verify every dimension before using it for estimating or ordering.</p></header>
    <section className="grid gap-4 rounded-2xl border bg-card p-4 md:grid-cols-3">
      <label className="text-sm font-medium">Save to<select className="mt-2 w-full rounded-lg border bg-background p-2" value={targetType} onChange={e=>{setTargetType(e.target.value);setTargetId("")}}><option value="project">Project</option><option value="estimate">Estimate</option></select></label>
      <label className="text-sm font-medium md:col-span-2">Destination<select className="mt-2 w-full rounded-lg border bg-background p-2" value={targetId} onChange={e=>setTargetId(e.target.value)}><option value="">Select {targetType}</option>{targets.map(t=><option key={t.id} value={t.id}>{t.project_number||t.estimate_number ? `${t.project_number||t.estimate_number} · `:""}{t.name||t.title}</option>)}</select></label>
    </section>
    <section className="grid gap-6 lg:grid-cols-[1.4fr_.6fr]">
      <div className="space-y-4 rounded-2xl border bg-card p-4"><div><h2 className="font-semibold">1. Capture the work area</h2><p className="text-sm text-muted-foreground">Keep the known reference and measured surface on the same plane for best results.</p></div><input aria-label="Take measurement photo" type="file" accept="image/*" capture="environment" onChange={e=>choosePhoto(e.target.files?.[0]||null)} className="block w-full text-sm"/>{photo&&<><canvas ref={canvasRef} onClick={tapCanvas} className="max-h-[65vh] w-full cursor-crosshair rounded-xl border object-contain"/><div className="flex flex-wrap gap-2 text-xs"><span className="rounded-full bg-amber-500/15 px-3 py-1">Tap 1–2: reference</span><span className="rounded-full bg-green-500/15 px-3 py-1">Tap 3–4: measurement</span><button onClick={resetPoints} className="rounded-full border px-3 py-1">Reset points</button></div></>}</div>
      <div className="space-y-4 rounded-2xl border bg-card p-4"><h2 className="font-semibold">2. Calibrate & verify</h2><label className="block text-sm">Known reference length (inches)<input type="number" min="0.01" step="0.01" value={reference} onChange={e=>setReference(Number(e.target.value))} className="mt-1 w-full rounded-lg border bg-background p-2"/></label><div className="rounded-xl border p-3"><p className="text-xs text-muted-foreground">Camera calculation</p><p className="text-2xl font-bold">{calculated?formatInches(calculated):"—"}</p></div><label className="block text-sm font-semibold">Verified measurement (inches)<input type="number" min="0.01" step="0.01" value={verified||""} onChange={e=>setVerified(Number(e.target.value))} className="mt-1 w-full rounded-lg border bg-background p-2"/><span className="mt-1 block text-xs font-normal text-muted-foreground">Correct this value with a tape/laser measurement when accuracy matters.</span></label></div>
    </section>
