...
function main() {
  ...
  assert(session.includes("Orion Operator architecture"), "Realtime policy preserves operator architecture for visible workflows");
  ...
  assert(session.includes("Fast form policy") && session.includes("Do not issue separate set calls") && session.includes("action=batch_set"), "Realtime policy tells Orion to collapse multi-field visible work into one tool round trip");
  assert(session.includes("Execution-speed policy") && session.includes("call the correct tool immediately") && session.includes("narrate briefly after the tool result"), "Realtime prioritizes execution over pre-action narration for reversible work");
  ...
  assert(session.includes("Direct-work fast path") && session.includes("bos_estimate_create"), "direct create/save requests can bypass form manipulation through canonical BOS services");
  assert(session.includes("Visible-form boundary") && session.includes("open, show, fill out, edit on screen, review, or watch the form"), "UI Operator is reserved for workflows where the user wants visible form interaction");
  assert(!session.includes("MANDATORY visible-create rule"), "legacy mandatory visible estimate creation rule stays removed");
  assert(session.includes("Visible estimate workflow") && session.includes("navigate to /estimates/new") && session.includes("batch-fill every compatible known value"), "visible estimate creation still opens and batch-fills the actual form when requested");
  assert(session.includes("Corrections override earlier information"), "normal conversational corrections update the active task");
