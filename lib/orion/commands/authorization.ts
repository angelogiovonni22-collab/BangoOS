...
export async function authorizeOrionCommand(input: {
  ...
  // Commands that do not yet have a semantic B.O.S. permission stay on their
  // existing command-specific role allowlist, but the role itself is always read
  // from the live membership row rather than trusted from an API payload/cache.
