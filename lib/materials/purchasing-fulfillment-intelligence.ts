...
export function summarizePurchaseOrderFulfillment(
  order: ProcurementPurchaseOrder,
  ...
  if (totals.damaged > 0) {
    ...
    risk = "attention";
    riskReason = `${totals.backordered} unit${totals.backordered === 1 ? "" : "s"} backordered.`;
  } else if (
    (order.status === "issued" || order.status === "partially_received") &&
    remaining > 0
  ) {
    risk = "attention";
    riskReason = `${remaining} unit${remaining === 1 ? "" : "s"} still outstanding.`;
  }
...
export function buildFulfillmentDashboard(payload: ProcurementOverviewPayload) {
  const orders = payload.purchaseOrders.map((order) => ({
    order,
    fulfillment: summarizePurchaseOrderFulfillment(order, payload.lineItems),
  }));
  return {
    orders,
    totals: {
      committedCost: roundMoney(
        orders
          .filter(({ order }) => order.status !== "cancelled")
          .reduce((sum, { fulfillment }) => sum + fulfillment.committedCost, 0),
      ),
      receivedCost: roundMoney(
        orders.reduce((sum, { fulfillment }) => sum + fulfillment.receivedCost, 0),
      ),
      outstandingUnits: orders.reduce(
        (sum, { fulfillment }) => sum + fulfillment.remainingQuantity,
        0,
      ),
