...
test("estimate signing incorporates versioned construction and electronic signature terms", () => {
  ...
  assert.match(agreement, /OHIO LAW CONTAINS IMPORTANT REQUIREMENTS/);
  assert.match(legal, /paper copy at no charge/);
  assert.match(legal, /electronic contact information changes/);
  assert.match(agreement, /B\.O\.S\. \(Bango Operating System\)/);
  assert.match(agreement, /OH-RC-1\.2/);
