  ...
  async function generateAgreementSnapshot(input: GenerateAgreementSnapshotInput) {
    ...
    const snapshot = {
      ...
      estimate: {
        ...
        depositValue: Number(estimate.deposit_value || 0),
        depositAmount: computeDepositAmount(estimate.deposit_type, Number(estimate.deposit_value || 0), Number(estimate.total_amount || 0)),
        terms: estimate.terms,
        paymentTerms: estimate.payment_terms,
        lineItems: lineItems || [],
      },
