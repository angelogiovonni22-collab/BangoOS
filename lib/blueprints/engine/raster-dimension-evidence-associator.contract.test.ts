...
const result = associateRasterDimensionEvidence({
  segments,
  dimensions: [singleDimension, splitDimension, unrelatedGapDimension, missingWitnessDimension],
...
assert.equal(split.start.x, 1);
assert.equal(split.end.x, 9);

assert(result.unresolvedDimensionIds.includes("dimension-unrelated-gap"), "collinear fragments must not bridge when the printed label does not occupy their gap");
assert(result.unresolvedDimensionIds.includes("dimension-missing-witness"), "a label-gap chain missing an outer witness must remain unresolved");
assert.equal(result.singleSegmentAssociationCount, 1);
assert.equal(result.labelGapChainAssociationCount, 1);
assert(result.diagnostics.some((item) => item.includes("arbitrary geometric gaps are never bridged")), "diagnostics must make fail-closed label-gap behavior explicit");
