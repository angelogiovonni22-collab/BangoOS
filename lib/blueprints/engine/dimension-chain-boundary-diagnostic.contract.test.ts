...
const provenance = [
  { dimensionId: "d1", rawText: "5m", sourceAssociationMode: "single_segment" as const, axis: "horizontal" as const, startCoordinate: 0, endCoordinate: 5, startCandidates: [wall("left", 0.01, 0.01)], endCandidates: [wall("good", 5.01, 0.01), wall("bad", 5.27, 0.27)] },
  { dimensionId: "d2", rawText: "3m", sourceAssociationMode: "single_segment" as const, axis: "horizontal" as const, startCoordinate: 2, endCoordinate: 5.02, startCandidates: [wall("mid", 2.01, 0.01)], endCandidates: [wall("good", 5.01, 0.01), wall("bad", 5.27, 0.25)] },
];
...
assert.equal(result.chains[0].recommendedCoordinate, 5.01);
