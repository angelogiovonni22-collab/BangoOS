...
function componentSummary(component:{wallFacePairs:MeterPair[];totalLength:number},index:number):BosSourceWallNetworkComponentSummary{
  let minX=Infinity,minY=Infinity,maxX=-Infinity,maxY=-Infinity;
  for(const pair of component.wallFacePairs){
    const a=Math.min(pair.axisStart,pair.axisEnd), b=Math.max(pair.axisStart,pair.axisEnd);
    if(pair.orientation==="horizontal"){ minX=Math.min(minX,a); maxX=Math.max(maxX,b); minY=Math.min(minY,pair.fixedMeters); maxY=Math.max(maxY,pair.fixedMeters); }
    else { minX=Math.min(minX,pair.fixedMeters); maxX=Math.max(maxX,pair.fixedMeters); minY=Math.min(minY,a); maxY=Math.max(maxY,b); }
  }
  return { componentId:`source-component-${index+1}`, pairCount:component.wallFacePairs.length, pairIds:component.wallFacePairs.map(p=>p.id), totalLengthMeters:component.totalLength, bounds:{minX,minY,maxX,maxY} };
}
...
export function selectSourceWallNetwork(input:{wallFacePairs:readonly BosSourceWallFacePair[];sourcePixelWidth:number;sourcePixelHeight:number;sourceWidthMeters:number;sourceHeightMeters:number;options?:BosSourceWallNetworkOptions}):BosSourceWallNetworkSelection{
  ...
  if(!structural.length)return{wallFacePairs:[],rejectedPairIds:pairs.map(p=>p.id),componentCount:0,retainedComponentCount:0,retainedComponents:[],repetitiveArtifactCount:repetitive.size,totalRetainedLengthMeters:0,diagnostics:["Rendered-source wall network selector retained no building-scale wall-face pairs."]};
  ...
  const retained=components.filter((c,i)=>i===0||(c.totalLength>=minComponent&&c.totalLength>=largest*relativeFloor)); const retainedIds=new Set(retained.flatMap(c=>c.wallFacePairs.map(p=>p.id))); const wallFacePairs=input.wallFacePairs.filter(p=>retainedIds.has(p.id)); const totalRetainedLengthMeters=wallFacePairs.reduce((s,p)=>s+p.lengthMeters,0);
  return {wallFacePairs,rejectedPairIds:input.wallFacePairs.filter(p=>!retainedIds.has(p.id)).map(p=>p.id),componentCount:components.length,retainedComponentCount:retained.length,retainedComponents:retained.map(componentSummary),repetitiveArtifactCount:repetitive.size,totalRetainedLengthMeters,diagnostics:[`Rendered-source wall network retained ${wallFacePairs.length} of ${input.wallFacePairs.length} paired wall-face candidates across ${retained.length} of ${components.length} connected components.`,`Rendered-source wall network rejected ${repetitive.size} repetitive short parallel pairs before component selection.`,`Largest independent source-wall component spans ${largest.toFixed(2)} m of paired wall evidence; opening continuity is limited to ${opening.toFixed(2)} m.`,`Retained component bounds and pair membership are exposed read-only so non-target drawing regions can be audited without changing selection.`]};
}
