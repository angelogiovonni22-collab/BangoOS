...
const building = [
  ...
  pair("top-right", "horizontal", 2, 6, 10),
...
const hatch = [
  pair("hatch-1", "horizontal", 14.0, 13, 14), pair("hatch-2", "horizontal", 14.3, 13, 14), pair("hatch-3", "horizontal", 14.6, 13, 14), pair("hatch-4", "horizontal", 14.9, 13, 14), pair("hatch-5", "horizontal", 15.2, 13, 14),
];
...
assert.equal(selection.retainedComponentCount, 1);
assert.equal(selection.retainedComponents[0].componentId, "source-component-1");
assert.deepEqual(selection.retainedComponents[0].bounds, { minX: 2, minY: 2, maxX: 10, maxY: 8 });
...
const titleRail = [
  pair("rail-spine-a", "vertical", 19.0, 1, 10),
  pair("rail-spine-b", "vertical", 19.05, 10, 19),
  pair("rail-top", "horizontal", 1, 17.2, 19.5),
  pair("rail-middle", "horizontal", 10, 17.2, 19.5),
  pair("rail-bottom", "horizontal", 19, 17.2, 19.5),
];
...
const retainedWithRail = new Set(withTitleRail.wallFacePairs.map((item) => item.id));
for (const item of building) assert(retainedWithRail.has(item.id), "architectural component must survive source-only side-rail rejection");
for (const item of titleRail) assert(!retainedWithRail.has(item.id), "narrow full-height right-side title-block rail must not become architectural source truth");
assert.equal(withTitleRail.rejectedSideRailPairCount, titleRail.length);
assert.equal(withTitleRail.retainedComponentCount, 1);
assert(withTitleRail.diagnostics.some((item) => item.includes("right-side title-block rails")));

assert(selection.diagnostics.some((item) => item.includes("opening continuity")));
assert(selection.diagnostics.some((item) => item.includes("component bounds")));
