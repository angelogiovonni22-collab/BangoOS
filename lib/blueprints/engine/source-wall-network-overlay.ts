...
function candidateCoverageMask(input: {
  ...
  for (const system of input.wallSystems) {
    for (const face of [system.faceA.line, system.faceB.line]) {
      const pixelLine: BosLine2 = {
        start: { x: face.start.x * pxX, y: face.start.y * pxY },
        end: { x: face.end.x * pxX, y: face.end.y * pxY },
      };
      for (const point of sampleLine(pixelLine)) {
        const cx = Math.round(point.x);
        const cy = Math.round(point.y);
        for (let dy = -input.radiusPixels; dy <= input.radiusPixels; dy += 1) {
          for (let dx = -input.radiusPixels; dx <= input.radiusPixels; dx += 1) {
...
/** Independent building-scale source-wall network coverage. */
export function assessSourceWallNetworkOverlay(input: {
  ...
  const candidateMask = candidateCoverageMask({
    image: input.image,
    wallSystems: input.wallSystems,
    sourceWidthMeters: input.sourceWidthMeters,
    sourceHeightMeters: input.sourceHeightMeters,
    radiusPixels: input.coverageRadiusPixels ?? 3,
  });
  ...
  const f1 = input.predictedPrecision + recall > 0
    ? 2 * input.predictedPrecision * recall / (input.predictedPrecision + recall)
    : 0;
  return {
    rawPairCount: faces.wallFacePairs.length,
    retainedPairCount: network.wallFacePairs.length,
    rejectedPairCount: faces.wallFacePairs.length - network.wallFacePairs.length,
    ...
    diagnostics: [
      `Candidate wall faces cover ${(recall * 100).toFixed(1)}% of independently paired, building-scale connected source-wall pixels (network F1 ${f1.toFixed(3)}).`,
      ...network.diagnostics,
      "Source-wall network selection uses only rendered-source wall-face pairs and never consults reconstructed candidate topology.",
    ],
  };
}
