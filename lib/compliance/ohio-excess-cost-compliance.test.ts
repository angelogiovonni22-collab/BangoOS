...
test("the estimate notice threshold is strictly greater than $5,000 cumulative", () => {
  const below = evaluateOhioExcessCostCompliance({
    ...base,
    currentExcessCostCents: OHIO_EXCESS_COST_NOTICE_THRESHOLD_CENTS - 1,
  });
  const exact = evaluateOhioExcessCostCompliance({
    ...base,
    currentExcessCostCents: OHIO_EXCESS_COST_NOTICE_THRESHOLD_CENTS,
  });
  const above = evaluateOhioExcessCostCompliance({
    ...base,
    currentExcessCostCents: OHIO_EXCESS_COST_NOTICE_THRESHOLD_CENTS + 1,
  });
...
test("smaller changes accumulate across the contract before the notice trigger", () => {
  const result = evaluateOhioExcessCostCompliance({
    ...base,
    priorQualifyingExcessCostCents: 450_000,
    currentExcessCostCents: 50_001,
  });
...
test("cost-plus contracts are exempt from the evaluated Chapter 4722 excess-cost gates", () => {
  const result = evaluateOhioExcessCostCompliance({
    ...
    currentExcessCostCents: 2_000_000,
  });
...
test("firm-price/no-excess contracts cannot be used to charge an excess amount", () => {
  const result = evaluateOhioExcessCostCompliance({
    ...base,
    contractEstimateMethod: "firm_price_no_excess",
    currentExcessCostCents: 1,
  });
...
test("unresolved classification fails closed for work and charging", () => {
  const result = evaluateOhioExcessCostCompliance({
    ...base,
    qualifiesAsReasonablyUnforeseenNecessary: null,
  });
...
test("a change classified outside the statutory unforeseen-and-necessary rule does not inflate the statutory cumulative amount", () => {
  const result = evaluateOhioExcessCostCompliance({
    ...base,
  ...
  assert.equal(result.cumulativeQualifyingExcessCostCents, 400_000);
  assert.equal(result.estimateNoticeRequired, false);
  assert.equal(result.chargeMayProceed, true);
});
