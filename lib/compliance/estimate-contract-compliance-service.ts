...
function profileFromRow(row: Record<string, unknown> | null): EstimateComplianceProfile {
  return {
    ...
    supplierTaxpayerId: (row?.supplier_taxpayer_id as string | null) || null,
    contractLanguage: (row?.contract_language as EstimateComplianceProfile["contractLanguage"]) || "unknown",
    ...
    insuranceCertificateUrl: (row?.insurance_certificate_url as string | null) || null,
    supplierSignerName: (row?.supplier_signer_name as string | null) || null,
    supplierSignedAt: (row?.supplier_signed_at as string | null) || null,
    supplierSignatureConfirmed: Boolean(row?.supplier_signature_confirmed),
...
export async function saveEstimateCompliance(
  ...
  const payload = {
    ...
    supplier_taxpayer_id: profile.supplierTaxpayerId?.trim() || null,
    contract_language: profile.contractLanguage || "unknown",
    ...
    insurance_certificate_url: profile.insuranceCertificateUrl?.trim() || null,
    supplier_signer_name: profile.supplierSignerName?.trim() || null,
    supplier_signature_confirmed: profile.supplierSignatureConfirmed === true,
    supplier_signed_at: profile.supplierSignatureConfirmed === true && profile.supplierSignerName?.trim() ? (profile.supplierSignedAt || new Date().toISOString()) : null,
