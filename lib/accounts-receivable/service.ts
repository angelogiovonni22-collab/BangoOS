...
export async function recordCustomerPayment(params:{supabase:SupabaseClient<Database>;companyId:string;userId:string;invoiceId:string;amount:number;paymentDate:string;method:string;referenceNumber?:string;notes?:string;}){
 ...
 const syncedInvoice=await params.supabase.from("invoices").select("amount_paid, status, paid_date").eq("company_id",params.companyId).eq("id",params.invoiceId).maybeSingle();if(syncedInvoice.error||!syncedInvoice.data){await params.supabase.from("invoice_payment_history").delete().eq("company_id",params.companyId).eq("id",paymentResult.data.id);return{error:syncedInvoice.error?.message||"Unable to confirm the updated invoice balance."};}
 ...
 return{error:null,isPaid,amountPaid:nextPaid,balanceDue};
}
