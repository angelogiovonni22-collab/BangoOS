...
export async function recordCustomerPayment(params:{supabase:SupabaseClient<Database>;companyId:string;userId:string;invoiceId:string;amount:number;paymentDate:string;method:string;referenceNumber?:string;notes?:string;}){
 ...
 const nextPaid=Math.min(total,alreadyPaid+params.amount),isPaid=nextPaid>=total-0.005,isDraft=invoiceResult.data.status==="draft";
 ...
 const nextStatus=isDraft?"draft":isPaid?"paid":"partially_paid";
 const updateResult=await params.supabase.from("invoices").update({amount_paid:nextPaid,status:nextStatus,paid_date:isPaid?params.paymentDate:null,updated_by:params.userId}).eq("company_id",params.companyId).eq("id",params.invoiceId).eq("updated_at",invoiceResult.data.updated_at).select("id").maybeSingle();if(updateResult.error||!updateResult.data){await params.supabase.from("invoice_payment_history").delete().eq("company_id",params.companyId).eq("id",paymentResult.data.id);return{error:updateResult.error?.message||"Invoice balance changed while recording the payment. Refresh and try again."};}
 ...
 await orion.publishEvent({company_id:params.companyId,actor_profile_id:params.userId,event_type:isPaid?"invoice.paid":"invoice.partial_payment",aggregate_type:"invoice",aggregate_id:params.invoiceId,source_module:"invoices",payload:{invoice_id:params.invoiceId,amount_received:params.amount,amount_paid:nextPaid,balance_due:Math.max(total-nextPaid,0),paid_date:isPaid?params.paymentDate:null,payment_date:params.paymentDate,deep_link:`/invoices/${params.invoiceId}`},metadata:{workflow_name:"accounts_receivable",event_category:"finance",event_severity:"success",deep_link:`/invoices/${params.invoiceId}`}});
 return{error:null,isPaid,amountPaid:nextPaid,balanceDue:Math.max(total-nextPaid,0)};
}
