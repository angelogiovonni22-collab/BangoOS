...
function displayCustomer(customer: CustomerSource | undefined) {
  ...
  return customer.customer_type?.toLowerCase() === "commercial" && company
    ? company
    : person || company || "Unnamed customer";
}
...
function dayDiff(from: Date, to: Date) {
  return Math.max(0, Math.floor((startOfDay(to).getTime() - startOfDay(from).getTime()) / 86_400_000));
}

export function resolveAgingBucket(dueDate: string | null, now = new Date()): AgingBucket {
...
export async function loadAccountsReceivable(
  ...
  companyId: string,
  now = new Date(),
): Promise<{ data: AccountsReceivableData | null; error: string | null }> {
  const monthStart = `${now.getFullYear()}-${String(now.getMonth() + 1).padStart(2, "0")}-01`;
  const [invoiceResult, customerResult, projectResult, paymentResult] = await Promise.all([
    supabase
      .from("invoices")
      .select("id, invoice_number, title, customer_id, project_id, status, issue_date, due_date, total_amount, amount_paid")
  ...
  const projects = new Map((projectResult.data ?? []).map((row) => [row.id, row]));

  const invoices: AccountsReceivableInvoice[] = (invoiceResult.data ?? [])
    ...
    .map((row) => {
      ...
      const bucket = resolveAgingBucket(row.due_date, now);
      const issued = row.issue_date ? parseDateOnly(row.issue_date) : startOfDay(now);
      return {
        id: row.id,
  ...
  const aging: AccountsReceivableSummary["aging"] = {
    current: 0,
    "1-30": 0,
    "31-60": 0,
    "61-90": 0,
  ...
  return {
    data: {
      ...
      summary: {
        totalReceivable,
        overdueReceivable: totalReceivable - currentReceivable,
        currentReceivable,
        collectedThisMonth,
        overdueCount: invoices.filter((row) => row.agingBucket !== "current").length,
...
export async function recordCustomerPayment(params: {
  ...
  if (["draft", "void", "paid"].includes(invoiceResult.data.status)) {
    return { error: "Payments can only be recorded against an open issued invoice." };
  }

  const total = Number(invoiceResult.data.total_amount || 0);
  const alreadyPaid = Number(invoiceResult.data.amount_paid || 0);
  const balance = Math.max(total - alreadyPaid, 0);
  if (params.amount > balance + 0.005) {
    return { error: "Payment cannot exceed the invoice balance." };
  }

  try {
    await authorizeInvoicePaymentCollection(params.supabase, {
      companyId: params.companyId,
      invoiceId: params.invoiceId,
      actorProfileId: params.userId,
      requestedAmount: params.amount,
      source: "accounts_receivable_manual_receipt",
    });
  } catch (error) {
    return { error: error instanceof Error ? error.message : "Payment compliance review is required." };
  }

  const nextPaid = Math.min(total, alreadyPaid + params.amount);
  const isPaid = nextPaid >= total - 0.005;
  ...
  const paymentResult = await params.supabase
    ...
    .insert({
      ...
      method: params.method.trim() || "manual",
  ...
  if (paymentResult.error) return { error: paymentResult.error.message };
  ...
  const updateResult = await params.supabase
    ...
    .update({
      ...
      paid_date: isPaid ? params.paymentDate : null,
      updated_by: params.userId,
    })
    .eq("company_id", params.companyId)
    .eq("id", params.invoiceId)
    .eq("amount_paid", alreadyPaid)
    .select("id")
    .maybeSingle();
  ...
  if (updateResult.error || !updateResult.data) {
    ...
    return { error: updateResult.error?.message || "Invoice balance changed while recording the payment. Refresh and try again." };
  }
  ...
  await orion.publishEvent({
    ...
    aggregate_id: paymentResult.data.id,
    ...
    payload: {
      ...
      resulting_balance: Math.max(total - nextPaid, 0),
      deep_link: `/invoices/${params.invoiceId}`,
    },
    metadata: {
      workflow_name: "accounts_receivable",
